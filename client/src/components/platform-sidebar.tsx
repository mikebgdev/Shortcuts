      ...
      <Card className="bg-white dark:bg-gray-800 border-slate-200 dark:border-gray-700">
        ...
        <CardContent className="space-y-2">
          {platforms.map((platform) => (
            ...
            >
              <i className={`${platform.icon} mr-2 group-hover:animate-bounce group-hover:text-blue-500 transition-all duration-300`}></i>
              {platform.name}
            </Button>
          ))}
        </CardContent>
          ...
          <div className="space-y-2 text-sm text-blue-800 dark:text-blue-300">
            {activePlatform === 'git' && (
              <div className="animate-fade-in">
                ...
                <p>Start with <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">git status</code> to check changes</p>
              </div>
            )}
            {activePlatform === 'docker' && (
              <div className="animate-fade-in">
                <p className="font-medium">🐳 Docker Tips:</p>
                <p>Use <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">docker ps</code> to see running containers</p>
              </div>
            )}
            {activePlatform === 'vim' && (
              <div className="animate-fade-in">
                <p className="font-medium">⌨️ Vim Basics:</p>
                <p>Press <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">i</code> to start editing, <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">Esc</code> to exit</p>
              </div>
            )}
            {activePlatform === 'phpstorm' && (
              <div className="animate-fade-in">
                <p className="font-medium">🚀 PHPStorm Pro:</p>
                <p>Use <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">Ctrl+Shift+N</code> for quick file access</p>
              </div>
            )}
            {(activePlatform === 'archlinux' || activePlatform === 'ubuntu') && (
              <div className="animate-fade-in">
                <p className="font-medium">🐧 Linux Power:</p>
                <p>Combine commands with <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">&&</code> for efficiency</p>
              </div>
            )}
          </div>
        </CardContent>

...
import {Check, Copy, Heart, Plus, StickyNote, Tag, X} from "lucide-react";
...
import type {Shortcut, TagType, UserNote} from "@/lib/types";
...
export default function EnhancedShortcutCard({
    ...
    const [shortcutTags, setShortcutTags] = useState<TagType[]>([]);
    ...
    // Fetch shortcut tags
    useEffect(() => {
        const fetchShortcutTags = async () => {
            try {
                const tags = await getShortcutTags(userId, shortcut.id);
                setShortcutTags(tags);
            } catch (error) {
                console.error('Error fetching shortcut tags:', error);
            }
        };
    ...
    const handleAddTag = async (tagId: number) => {
        setIsLoading(true);
        try {
            await addShortcutTag(userId, shortcut.id, tagId.toString());
            const updatedTags = await getShortcutTags(userId, shortcut.id);
            setShortcutTags(updatedTags);
    ...
    const handleRemoveTag = async (tagId: number) => {
        setIsLoading(true);
        try {
            await removeShortcutTag(userId, shortcut.id, tagId.toString());
            const updatedTags = await getShortcutTags(userId, shortcut.id);
            setShortcutTags(updatedTags);
    ...
    const handleAddTagSelect = (tagId: string) => {
        ...
        <Card className="hover:shadow-lg transition-all duration-200 group border-l-4"
                    ...
                    <div className="flex items-center gap-1 ml-2">
                        ...
                        <Dialog open={tagDialogOpen} onOpenChange={setTagDialogOpen}>
                            ...
                            <DialogContent>
                                ...
                                <Select onValueChange={handleAddTagSelect}>
                                    <SelectTrigger>
            ...
            <CardContent className="pt-0">
                <div className="space-y-3">
                    ...
                    <div
                        className="bg-gradient-to-r from-gray-900 to-gray-800 dark:from-gray-800 dark:to-gray-900 rounded-lg p-4 shadow-inner">
                        <div className="flex items-center justify-between">
                            <code
                                className="text-green-400 dark:text-green-300 font-mono text-base font-semibold tracking-wide">
                    ...
                    {userNote && (
                        <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-950/20 rounded border-l-4 border-blue-500">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span
                                    className="text-sm font-medium text-blue-700 dark:text-blue-300">Mi Nota Personal</span>
                            </div>
